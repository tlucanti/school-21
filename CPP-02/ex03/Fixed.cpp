...
/*   Updated: 2022/01/08 23:29:55 by kostya           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#endif
	_number = round(val * (float)(1 << _frac));
}
...
//	return Fixed(toFloat() * other.toFloat());
	return FixedPrivateConstructor
	(
		_number * (other._number >> _frac) +
		(int)(_number & ~((unsigned)-1) << _frac)
	);
}
...
int
tlucanti::Fixed::round(float v) __CONST
{
	return (int)roundf(v);
}
