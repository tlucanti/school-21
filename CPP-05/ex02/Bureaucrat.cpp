...
/*   Updated: 2022/01/16 13:03:30 by kostya           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void
tlucanti::Bureaucrat::signForm(Form &frm) const
{
	try
	{
		frm.beSigned(*this);
	...
	catch (const tlucanti::BureaucratException &exc)
	{
		...
		std::cout << "<" << name << "> cannot sign form <" << frm.getName() <<
			">\n";
		return ;
	}
	std::cout << "<" << name << "> signed form <" << frm.getName() <<
		"> successfully\n";
}
